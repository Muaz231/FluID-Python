...
receiver = phased.ReceiverPreamp('Gain',30,'NoiseFigure',4.5,'SampleRate',fs);
...
car_dist = [20 40];              % Distance between sensor and cars (meters)
car_speed = [0 0]*1000/3600;  % km/h -> m/s
car_az = [-50, +40]; % Ground Truth AoA in deg
car_rcs = [50, 50];
car_pos = [car_dist.*cosd(car_az);car_dist.*sind(car_az);0 0];
...
%% MUSIC Code Added to MATLAB's tutorial code
Nsources = 2;
sep = lambda/2;
[p_music,D,sigspace,nspace] = MUSIC(xrv(:,:,1).',Nsources,Nr*Nt,sep);

figure(1);
angle = linspace(-90,90,180);
plot(angle,p_music,'LineWidth',2,'LineStyle','-')
xlabel('Angles $\theta \circ$ (deg)','Interpreter','latex','FontSize',10)
ylabel('AoA Power Spectrum','FontSize',10)
title('MUSIC Spectrum')
grid minor
xticks((-90:10:90))
legend(strcat('Ground Truth Azimuth Angle(deg) : ', num2str(car_az)))
%% Range Doppler Processing and BeamScanEstimator
...
%% MUSIC Function
% Function to estimate AoA using MUSIC algorithm
function [p_spectrum, Dhat, signalSpaceMtx, noiseSpaceMtx] = MUSIC(recv_sigm, Ns, M, sep)
% Function to implement MUSIC algorithm for estimating the AoA
% *TODOs*:
% a) Compute the correlation matrix of the received signal
% b) Compute the eignenvalues and eigenvectors of the correlation matrix
% c) Extract the eigenvectors corresponding to the noises
% d) Define the steering vector
% e) Loop over the input angle [-90, 90] and compute the pseudo spectrum
Nangle = 180;
p_spectrum = zeros(Nangle,1);

corr_matrix = recv_sigm * recv_sigm';
corr_matrix = corr_matrix + conj(flip(flip(corr_matrix, 1), 2));
corr_matrix = corr_matrix / 2;

[V,D] = eig(corr_matrix);


[sort_val, sort_idx]=sort(abs(diag(D)),'descend');
Dhat =diag(D); Dhat = Dhat(sort_idx);
sort_eig=V(:,sort_idx);
noiseSpaceMtx=sort_eig(:, Ns+1:end);
noiseSpaceMtxH=noiseSpaceMtx';

signalSpaceMtx = sort_eig(:,1:Ns);

angles = linspace(-90,90,Nangle);
for ang_index = (1:numel(angles))
    a = exp(-1i .* 2 * pi * (sep/(2*sep)) .* (0:M-1)' .* sind(angles(ang_index)) );
   p_spectrum(ang_index) = 1 ./ vecnorm(noiseSpaceMtxH * a, 2, 1);
end
